...
int main(int argc, char* argv[])
{
	...
	while ((c = getopt (argc, argv, "hVv::" "c:H:w:T:A:D:C:" "t:S::s::" "a::q:" "n:u:U:")) != -1)
	{
		switch (c)
		{
			...
			case 'c': sslCertificate = optarg; config["ssl_certificate"] = optarg; break;
			case 'w': webroot = optarg; config["document_root"] = optarg; break;
			case 'T': nbthreads = optarg; config["num_threads"] = atoi(optarg);break;
			case 'A': passwdFile = optarg; config["global_auth_file"] = optarg; break;
			case 'D': authDomain = optarg; config["auth_domain"] = optarg; break;

			case 't': turnurl = optarg; config["turn_url"] = optarg; break;
			case 'S': localstunurl = optarg ? optarg : defaultlocalstunurl; stunurl = localstunurl; 
				config["stun_url"] = stunurl;
				break;
			case 's': localstunurl = NULL; stunurl = optarg ? optarg : defaultlocalstunurl; 
				config["stun_url"] =stunurl;
			break;
			
			case 'a': audioLayer = optarg ? (webrtc::AudioDeviceModule::AudioLayer)atoi(optarg) : webrtc::AudioDeviceModule::kDummyAudio; 
			config["audio_layer"] = audioLayer; 
			break;
			case 'q': publishFilter = optarg ; config["publish_filter"] = optarg; break;
	...
	config["listening_ports"] = httpAddress;
	
#if 1
	// Create WebRTCStreamer server using json config.
	
	WebRTCStreamer streamer(config);
	// Set the streams and positions, if any.
	streamer.setStreams(urlVideoList, urlAudioList, positionList);
	if (streamer.init())
		streamer.run();
